...
class AlpacaExchange(ExchangeAdapter):
    ...
    async def get_historical_closes(
        ...
        *,
        ...
        # start/end 가 주어지면 그대로 사용, 아니면 limit 기반 자동 계산
        if start is None or end is None:
            lookback_days = {
                "1m": max(1, limit // (60 * 6)) * 2,
                "5m": max(1, limit // 78) * 2,
        ...
        # Alpaca 는 일봉 이상은 YYYY-MM-DD, 분봉은 ISO 8601 권장
        date_fmt = "%Y-%m-%d" if interval in ("1d",) else "%Y-%m-%dT%H:%M:%SZ"
        all_bars: list[dict] = []
        ...
        while True:
            ...
            r = await self._data_client.get(f"/v2/stocks/{symbol}/bars", params=params)
            r.raise_for_status()
            data = r.json()
        ...
        return [Decimal(str(b["c"])) for b in all_bars[-limit:]]
